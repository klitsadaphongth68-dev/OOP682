...
class InMemoryTaskRepository(ITaskRepository):
    ...
    def update(self, task_id: int) -> Optional[Task]:
        for index, task in enumerate(self.tasks):
            if task.id == task_id:
                updated_task = Task(id=task.id, title=task.title, description=task.description, completed=True)
...
class SqlTaskRepository(ITaskRepository):
    ...
    def update(self, task_id: int) -> Optional[Task]:
        db_task = self.db.query(models_orm.TaskORM).filter(models_orm.TaskORM.id == task_id).first()
        if not db_task:
            return None
